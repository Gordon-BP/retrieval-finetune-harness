...
class ReRankEvalPipeline:
    ...
    def __init__(self, bi_encoder, cross_encoder, corpus, ):
    ...
    def _search(self, query_embedding, method, k):
        if method == "cosine":
            ...
            top_results = torch.topk(-distances, k=k)  # Negative because we want smallest distances
            score = top_results[0]
            return top_results.indices.cpu()
    ...
    def search(self, query, k, method="cosine"):
        ...
        results = [self.corpus[top_hits[i]] for i, score in enumerate(scores) if score > CROSS_ENCODER_THRESHOLD]
        if(len(results)==0):
