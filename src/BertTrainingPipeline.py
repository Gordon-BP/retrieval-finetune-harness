...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        anchor_inputs = self.tokenizer(anchor, return_tensors="pt", truncation=True, padding=False)
        positive_inputs = self.tokenizer(positive, return_tensors="pt", truncation=True, padding=False)
        negative_inputs = self.tokenizer(negative, return_tensors="pt", truncation=True, padding=False)
...
class BertTrainingPipeline:
    ...
    def collate_fn(self,batch):
        ...
        anchor_input_ids = torch.nn.utils.rnn.pad_sequence([item['input_ids'].squeeze() for item in anchor_batch], batch_first=True)
        anchor_attention_mask = torch.nn.utils.rnn.pad_sequence([item['attention_mask'].squeeze() for item in anchor_batch], batch_first=True)
        positive_input_ids = torch.nn.utils.rnn.pad_sequence([item['input_ids'].squeeze() for item in positive_batch], batch_first=True)
        positive_attention_mask = torch.nn.utils.rnn.pad_sequence([item['attention_mask'].squeeze() for item in positive_batch], batch_first=True)
        negative_input_ids = torch.nn.utils.rnn.pad_sequence([item['input_ids'].squeeze() for item in negative_batch], batch_first=True)
        negative_attention_mask = torch.nn.utils.rnn.pad_sequence([item['attention_mask'].squeeze() for item in negative_batch], batch_first=True)
        return {'anchor': {'input_ids': anchor_input_ids, 'attention_mask': anchor_attention_mask},
    ...
    def train(self, BI_ENCODER_EPOCHS,
      ...
      scheduler = optim.lr_scheduler.LambdaLR(optimizer, lambda x:self.lr_lambda(x, len(train_dataloader),BI_ENCODER_WARMUP_MULT))
